...
FROM scratch
COPY . /
RUN opkg update && opkg install iptables iptables-zz-legacy bind-dig tcpdump vim-fuller luci rsync unbound-daemon chrony-nts ntpdate luci-app-ddns wireguard-tools luci-proto-wireguard qrencode netcat luasocket
CMD ["/bin/sh"]
EOF
